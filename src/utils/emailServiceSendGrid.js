...
const sendVerificationEmail = async (user, password) => {
  try {
    ...
    if (!process.env.SENDGRID_API_KEY) {
      console.error('SendGrid API key missing. Check SENDGRID_API_KEY environment variable.');
      return false;
    }
    ...
    const mailOptions = {
      ...
      from: {
        ...
        name: process.env.EMAIL_FROM_NAME || 'Errbud Platform'
      },
    ...
    console.log(`✅ Verification email sent successfully to ${user.email} via SendGrid`);
    return true;
  } catch (error) {
